...
class WaveOverlay(QMainWindow):
    def __init__(self):
        ...
        html_path = os.path.abspath(os.path.join(assets_dir, "wave.html"))
...
def _invoke(obj, method_name, connection_type):
    obj.metaObject().invokeMethod(obj, method_name, connection_type)
...
def listen_commands(app, overlay):
    ...
    try:
        for line in sys.stdin:
            ...
            if cmd == "show":
                _invoke(overlay, "show_overlay", Qt.ConnectionType.QueuedConnection)
            elif cmd == "hide":
                _invoke(overlay, "hide_overlay", Qt.ConnectionType.QueuedConnection)
            elif cmd == "exit":
                _invoke(app, "quit", Qt.ConnectionType.QueuedConnection)
                break
    except Exception:
        pass
    finally:
