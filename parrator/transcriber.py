...
class Transcriber:
    ...
    def load_model(self) -> bool:
        ...
        try:
            ...
            if required_version and not self._is_onnx_asr_version_supported(
                required_version
            ):
                self.last_error = (
                    f"Модель '{model_name}' требует "
                    f"onnx-asr>={required_version}. "
                    "Обновите зависимость и перезапустите "
                    "приложение."
                )
                self._log(self.last_error)
                return False
            ...
            if model_path_resolved:
                ...
                self.model = load_model(
                    model_name, path=model_path_resolved, providers=providers
                )
            else:
    ...
    def _transcribe_whisper_with_vad(self, audio_path: str) -> list[str]:
        ...
        try:
            segments = list(
                self.model.with_vad(self.vad, batch_size=1).recognize(audio_path)
            )
        except Exception as e:
        ...
        if self._has_oversized_vad_segments(segments):
            self._log(
                "Whisper VAD returned oversized segments, switching to window fallback"
            )
            return []
    ...
    def _recognize_text(
        self, audio_input: str | np.ndarray, sample_rate: int = 16000
    ) -> str:
