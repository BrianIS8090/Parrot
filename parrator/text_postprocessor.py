...
class TextPostProcessor:
    ...
    def __init__(
        ...
        self._morph = (
            ...
            else self._create_morph_analyzer()
        )
    ...
    def _create_morph_analyzer(self) -> Optional[Any]:
        try:
            ...
            self._log(
                "pymorphy3 недоступен, используется fallback на точные замены словаря"
            )
            return None
    ...
    def _load_dictionary(self) -> Dict[str, str]:
        ...
        if isinstance(raw_dictionary, dict):
            ...
            self._log(
                "Поле 'dictionary' в конфиге "
                'должно быть объектом вида {"что": "на что"}'
            )
        ...
        try:
            ...
            if isinstance(file_dictionary, dict):
                ...
                self._log(
                    'Файл словаря должен содержать JSON-объект вида {"что": "на что"}'
                )
    ...
    def _replace_words(
        self, text: str, exact_map: Dict[str, str], lemma_map: Dict[str, str]
    ) -> str:
